...
export default function AccountDetail() {
  ...
  const loadAccount = useCallback(async () => {
    ...
    try {
      ...
      const accountResponse = await fetch(`${getApiUrl()}/api/accounts`, {
        headers: {
      ...
      const positionsResponse = await fetch(
        `${getApiUrl()}/api/accounts/${id}/positions`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
  ...
  const handleUpdatePosition = async (positionId: string) => {
    ...
    try {
      const token = await getToken();
      const response = await fetch(`${getApiUrl()}/api/positions/${positionId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
    ...
    return (
      <Layout>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="border border-[var(--border)] bg-[var(--card)] rounded-lg shadow p-6">
            <p className="text-center text-[var(--text-secondary)]">Loading account details...</p>
          </div>
        </div>
      </Layout>
    );
  ...
  return (
    <Layout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Account Details */}
        <div className="border border-[var(--border)] bg-[var(--card)] rounded-lg shadow p-6 mb-6">
          <div className="flex justify-between items-start mb-6">
            <div className="flex-1">
              {editingAccount ? (
                <div className="space-y-4 max-w-md">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className="rounded-lg border border-[var(--border)] bg-[var(--surface)] px-4 py-2 text-[var(--text-secondary)] transition-colors hover:bg-[var(--card)]"
                    >
                      Cancel
                    </button>
                  </div>
                ...
                <>
                  <h2 className="text-2xl font-bold text-[var(--text-primary)]">{account.account_name}</h2>
                  <p className="text-[var(--text-secondary)] mt-1">{account.account_purpose}</p>
                </>
              )}
            </div>
            {!editingAccount && (
          ...
          {/* Account Summary */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-[var(--surface)] rounded-lg">
            <div>
            ...
            <div>
              <p className="text-sm text-[var(--text-secondary)]">Positions Value</p>
              <p className="text-lg font-semibold">
        ...
        <div className="border border-[var(--border)] bg-[var(--card)] rounded-lg shadow p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-xl font-bold text-[var(--text-primary)]">Positions</h3>
            <button
          ...
          {positions.length === 0 ? (
            <div className="text-center py-8 bg-[var(--surface)] rounded-lg">
              <p className="text-[var(--text-secondary)]">No positions in this account yet</p>
              <p className="text-sm text-[var(--text-secondary)] mt-2">Click &ldquo;Add Position&rdquo; to start building your portfolio</p>
            </div>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[var(--border)]">
                    <th className="text-left py-3 px-4 font-semibold text-[var(--text-secondary)]">Symbol</th>
                    <th className="text-right py-3 px-4 font-semibold text-[var(--text-secondary)]">Quantity</th>
                    <th className="text-right py-3 px-4 font-semibold text-[var(--text-secondary)]">Price</th>
                    <th className="text-right py-3 px-4 font-semibold text-[var(--text-secondary)]">Value</th>
                    <th className="text-center py-3 px-4 font-semibold text-[var(--text-secondary)]">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {positions.map((position) => (
                    <tr key={position.id} className="border-b border-[var(--border)] hover:bg-[var(--surface)] transition-colors">
                      ...
                      <td className="py-4 px-4 text-right">
                        {editingPosition === position.id ? (
                          <input
                            ...
                            className="w-24 border border-[var(--border)] rounded px-2 py-1 text-right focus:outline-none focus:ring-2 focus:ring-primary"
                            step="0.01"
                            min="0"
                          />
                        ) : (
                          ...
                          {editingPosition === position.id ? (
                            <>
                              ...
                              <button
                                ...
                                className="text-[var(--text-secondary)] hover:bg-[var(--surface)] p-2 rounded transition-colors"
                                title="Cancel"
                              >
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {showAddPosition && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--overlay)] p-4">
            <div className="border border-[var(--border)] bg-[var(--card)] rounded-lg max-w-md w-full p-6">
              <h3 className="text-xl font-bold text-[var(--text-primary)] mb-4">Add New Position</h3>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full border border-[var(--border)] rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary uppercase"
                      placeholder="Enter ticker symbol (e.g., SPY, AAPL)"
                    />

                    {showSymbolSuggestions && filteredInstruments.length > 0 && (
                      <div className="absolute z-10 w-full mt-1 rounded-lg border border-[var(--border)] bg-[var(--card)] shadow-lg max-h-48 overflow-y-auto">
                        {filteredInstruments.map((inst) => (
                          <button
                            ...
                            className="w-full text-left px-3 py-2 hover:bg-[var(--surface)] border-b border-[var(--border)] last:border-b-0"
                          >
        ...
        <ConfirmModal
          ...
          message={
            <div>
              ...
              <p className="text-sm mt-2 text-[var(--text-secondary)]">This will remove this holding from your account.</p>
