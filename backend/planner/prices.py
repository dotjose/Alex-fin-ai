...
try:
    ...
    RESTClient = None  # type: ignore[misc, assignment]
...
polygon_plan = os.getenv("POLYGON_PLAN") or "free"

is_paid_polygon = polygon_plan == "paid"


def is_market_open() -> bool:
    if RESTClient is None or not polygon_api_key:
        logger.warning("Polygon not available, assuming market closed")
        return False
...
def get_all_share_prices_polygon_eod() -> dict[str, float]:
    ...
    if RESTClient is None or not polygon_api_key:
        logger.warning("Polygon not available, skipping grouped daily aggs")
        return {}
    client = RESTClient(polygon_api_key)
...
def get_share_price(symbol) -> float:
    if polygon_api_key and RESTClient is None:
        logger.warning(
            "Polygon SDK not installed (polygon-api-client); skipping market data for %s",
            symbol,
        )
        return float(random.randint(1, 100))
    if polygon_api_key and RESTClient is not None:
        try:
